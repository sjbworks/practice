const addTwoNumbers = (l1: number[], l2: number[]) => {
  ...
  for (let i = 0; i < arrayLong.length; i++) {
    ...
    const tempString: string = temp > 9 ? String(temp).substring(1, 2) : String(temp)
    arrayResult.push(Number(tempString))
    temp > 9 && tenPlace.push(Number(String(temp).substring(0, 1)))
    i === arrayLong.length - 1 && tenPlace.length && arrayResult.push(tenPlace.pop())
  }
  ...
  for (let i = 0; i < s.length; i++) {
    for (let j = 0; j <= s.length; j++) {
      if (j > i) {
        ...
        if (str === str.split('').reverse().join('') && palindrome.length < str.length) {
          palindrome = str
        }
      }
    }
